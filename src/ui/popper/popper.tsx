...
import { alignBox, cn, navigateItems } from "../../utils";
import {
  ANIMATION_DURATION,
  ANIMATION_TIMEOUT,
...
export const PopperContext = React.createContext<PopperContextProps | null>(
  null,
);
const PopperRadioGroupContext =
  React.createContext<PopperRadioGroupContextProps | null>(null);
...
const usePopperRadioGroup = () => {
  const context = React.useContext(PopperRadioGroupContext);
  if (!context)
...
>(({ children, className = undefined, asChild, style }, forwardRef) => {
  ...
  }, [setTriggerPosition, ref.current]);
...
const PopperTrigger = React.forwardRef<HTMLElement, PopperTriggerProps>(
  (
    ...
    return asChild && React.isValidElement(children) ? (
      React.cloneElement(children, commonAttributes)
    ) : menuType === "select" ? (
      <button
        {...(commonAttributes as React.HTMLAttributes<HTMLButtonElement>)}
        className={cn(
...
const PopperContent = ({
  ...
  }, [align, menuType, position, triggerPosition, ref.current, setMenuStyle]);
...
const PopperSeparator = React.forwardRef<HTMLDivElement, PopperSeparatorProps>(
...
const PopperLabel = React.forwardRef<HTMLLabelElement, PopperLabelProps>(
  (
