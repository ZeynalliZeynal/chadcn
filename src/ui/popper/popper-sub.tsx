...
import { useDebounce, useOutsideClick, useResize } from "../../hooks";
import {
  ANIMATION_DURATION,
  ANIMATION_TIMEOUT,
...
import { cn, navigateItems } from "../../utils";
...
type PopperContextSubProps = {
  ...
  setTriggerPosition: React.Dispatch<React.SetStateAction<DOMRect | undefined>>;
  animate: boolean;
...
const PopperSub = ({ children }: { children: React.ReactNode }) => {
  ...
  const [position, setPosition] = React.useState<DOMRect | undefined>(
    undefined,
  );
  ...
  const openSubPopper = React.useCallback(
    ...
    [clearDebounce, setAnimate, setOpenSub, setPosition, setActiveTrigger],
  );

  const closeSubPopper = React.useCallback(() => {
    setAnimate(true);
    debounce(() => {
      setOpenSub(false);
      setAnimate(false);
  ...
  }, [debounce, setAnimate, setOpenSub, setActiveTrigger]);
...
const PopperSubTrigger = ({
  ...
  const { openSubPopper, closeSubPopper, openSub, setCurrentItemIndex } =
    usePopperSub();
  const [openedWithKey, setOpenedWithKey] = React.useState(false);
